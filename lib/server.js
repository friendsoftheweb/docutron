...
app.get('/:document/:version/:section?.html', function(req, res) {
  ...
  const templateFilePath = path.join(
    process.cwd(),
    'documents',
    section ? `${document}/${section}.hbs` : `${document}/index.hbs`
  );

  getData(document, version, (error, data) => {
    if (error == null) {
      // Merge query params into data to support page numbers in headers/footers
      data = Object.assign({}, { document: req.query }, data);
...
app.get('/:document/:version.pdf', function(req, res) {
  ...
  const command = wkhtmltopdfCommand(`http://localhost:${app.get('port')}`, document, version);
