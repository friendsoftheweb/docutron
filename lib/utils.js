...
function wkhtmltopdfCommand(host, document, version) {
  ...
  const coverPath = path.join(WORKING_DIRECTORY, `documents/${document}/cover.mustache`);
  ...
  const headerPath = path.join(WORKING_DIRECTORY, `documents/${document}/header.mustache`);
  const footerPath = path.join(WORKING_DIRECTORY, `documents/${document}/footer.mustache`);
...
function renderTemplate(filePath, view = {}) {
  ...
  glob.sync(path.join(WORKING_DIRECTORY, 'documents/shared/**/*.mustache')).forEach(partialPath => {
