...
function documents() {
  ...
  return fs
    ...
    .filter(
      (file) =>
        fs.statSync(path.join(documentsDir, file)).isDirectory() &&
        file !== 'shared'
    );
}
...
function wkhtmltopdfCommand(host, document, version) {
  ...
  const headerPath = path.join(
    WORKING_DIRECTORY,
    `documents/${document}/header.hbs`
  );
  const footerPath = path.join(
    WORKING_DIRECTORY,
    `documents/${document}/footer.hbs`
  );

  if (fs.existsSync(headerPath)) {
    options.push(
      `--header-html http://localhost:3000/${document}/${version}/header.html`
    );
  }

  if (fs.existsSync(footerPath)) {
    options.push(
      `--footer-html http://localhost:3000/${document}/${version}/footer.html`
    );
  }
...
function renderTemplate(filePath, data = {}) {
  ...
  glob
    .sync(path.join(WORKING_DIRECTORY, 'documents/shared/**/*.hbs'))
    .forEach((partialPath) => {
